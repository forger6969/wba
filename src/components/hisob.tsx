...
export function HisobForma({
  ...
  const taklif = turi === 'oquvchi' ? `${nishon.toLowerCase()}@wba.uz` : ''
  ...
  return (
    <details className="rounded-[10px] border border-dashed border-line px-4 py-3">
      <summary className="flex cursor-pointer items-center gap-2 text-[13px] font-semibold text-ink-2">
        ...
        {bormi && <Badge ton="ok">hisobi bor{email ? ` · ${email}` : ''}</Badge>}
      </summary>
      ...
      <form action={hisobOch} className="mt-3 grid gap-3 sm:grid-cols-[1.4fr_1.2fr_auto] sm:items-end">
        ...
        <Maydon nom="Email" izoh="Kirish uchun. Haqiqiy pochta bo‘lishi shart emas.">
          <input
            name="email"
            type="email"
            required
            defaultValue={email ?? taklif}
            placeholder="aziza@wba.uz"
      ...
      <p className="mt-2 text-[11.5px] leading-relaxed text-ink-3">
        {ism} shu email va parol bilan kiradi va faqat{' '}
