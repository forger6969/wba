...
4. "Terminal": Q3 = a bo'lsa `USUL_MAP.terminal = 'karta'`; b bo'lsa `<keyingi bo'sh raqam>_usul_terminal.sql` (0018–0021 band: ota-ona, panel, telegram) (`alter type payment_method add value 'terminal'`, alohida fayl) + `src/lib/types.ts` + to'lov formasi.
...
- Q1 = b (kelgan kuni): `<keyingi bo'sh raqam>_hisob_oyma_oy.sql`: `create_monthly_invoices` o'rniga har kuni "bugun oy-yubileyi kelgan" yozilishlarga navbatdagi oy hisobini yozadigan funksiya; `oy_raqami` DATEDIF mantig'ida. `_test_logic.sql` ga: 16.09 boshlangan — 15.10 da 1 hisob, 16.10 da 2.
